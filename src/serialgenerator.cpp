...
SerialGenerator::SerialGenerator() :
    prototypeSerial_("??????????"),  // Default constructor initializes an 10-digit generator
...
QString SerialGenerator::generateSerial() const
{
    ...
    while (serial.contains('?')) {
        serial[serial.indexOf('?')] = replaceWith.at(QRandomGenerator::global()->bounded(bounds));
    }
    return serial;
}
...
{
    return static_cast<size_t>(prototypeSerial.size()) <= MCP2221::DESC_MAXLEN && prototypeSerial.contains('?');
}
